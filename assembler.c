...
void		ft_usage(void)
{
	...
	i = -1;
	ft_printf(GREEN("****** THE USAGE OF ASM *****\n\n"));
	ft_printf("The ASM program compiling \".s\" "
	"extention files into byte code for ");
	ft_printf("CoreWar championship.\n");
	ft_printf("For detailed information please"
	" visit https://en.wikipedia.org/wiki/Core_War\n\n");
	ft_printf("The following operations are applicable via compilation:\n");
	while (++i < 16)
		ft_printf("%8s | %s\n", op_tab[i].name, op_tab[i].desc);
	ft_printf("\n");
	ft_exit(0);
}
