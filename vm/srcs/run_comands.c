...
void 	process_add_run(t_vm *vm, t_process *pr, t_runner *run)
{
	...
	res = read_varlen_be(pr->regs[run->args[A1] - 1], sizeof(pr->regs[0]), SIGNED);
	res += read_varlen_be(pr->regs[run->args[A2] - 1], sizeof(pr->regs[0]), SIGNED);
	write_varlen_be(pr->regs[run->args[A3] - 1], res, sizeof(pr->regs[0]));
	pr->carry = res == 0;
}
...
void 	process_and_run(t_vm *vm, t_process *pr, t_runner *run)
{
	...
	if (run->types[A1] == REG_CODE)
		val1 = read_varlen_be(pr->regs[run->args[A1] - 1], sizeof(pr->regs[0]), UNSIGNED);
...
void 	process_ldi_run(t_vm *vm, t_process *pr, t_runner *run)
{
	...
	if (run->types[A1] == REG_CODE)
		val1 = read_varlen_be(pr->regs[run->args[A1] - 1], sizeof(pr->regs[0]), SIGNED);
	else if (run->types[A1] == DIR_CODE)
		val1 = run->args[A1];
	else
	...
	write_varlen_be(pr->regs[run->args[A3] - 1], val, DIR_SIZE);
}
...
void 	process_lldi_run(t_vm *vm, t_process *pr, t_runner *run)
{
	...
	val = read_be_map(vm->field, pr->pc + (val1 + val2), DIR_SIZE, UNSIGNED);
	write_varlen_be(pr->regs[run->args[A3] - 1], val, DIR_SIZE);
}
